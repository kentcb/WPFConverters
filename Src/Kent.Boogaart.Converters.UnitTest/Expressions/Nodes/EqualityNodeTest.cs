    ...
    public sealed class EqualityNodeTest : UnitTest
    {
        ...
        public void IsSupported_ShouldYieldTrueIfBothStringsOrBothNumeric()
        {
            foreach (NodeValueType type1 in Enum.GetValues(typeof(NodeValueType)))
            {
                ...
                {
                    Assert.True(InvokePrivateMethod<bool>(_equalityNode, "IsSupported", type1, type2));
                }
            }
        }
        ...
        public void DoValueType_ShouldDoComparison()
        {
            ...
            Assert.True(InvokePrivateMethod<bool>(_equalityNode, "DoValueType", value1, value3));
            Assert.False(InvokePrivateMethod<bool>(_equalityNode, "DoValueType", value1, value2));
            Assert.False(InvokePrivateMethod<bool>(_equalityNode, "DoValueType", value3, value2));
        }

        [Fact]
        public void DoReferenceType_ShouldDoComparison()
        {
            Assert.True(InvokePrivateMethod<bool>(_equalityNode, "DoReferenceType", this, this));
            Assert.False(InvokePrivateMethod<bool>(_equalityNode, "DoReferenceType", this, this._equalityNode));
            Assert.False(InvokePrivateMethod<bool>(_equalityNode, "DoReferenceType", this._equalityNode, new EqualityNode(new ConstantNode<int>(0), new ConstantNode<int>(0))));
        }
    }
}
